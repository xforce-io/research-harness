...
- **`experience` = 右轴闭环**：把执行变成可学习介质（捕获流 → 更新权重或 harness → 评价）。更新步在本闭环内，不另立 `evolution` 支柱。
- **`security` = 左横切轴**；懒创建。
...
- **Mandate**：生产交互流的捕获、分诊、更新与评价。介质是 agent 自身经验，不是更多人类语料或新 trainer（Silver & Sutton *Era of Experience*；姚顺雨 *The Second Half*：评价与任务定义重于再发明 recipe）。
